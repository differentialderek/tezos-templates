...
type transfer = { to_ : address ; amt_ : nat ; }
...
let transfer (param : transfer) (storage : storage) : result = 
    let (from_, to_, amt_) = (Tezos.source, param.to_, param.amt_) in 
    // fetch the sender's and recipient's balance 
    let (sender_bal, recipient_bal, ops_storage_updates) = 
        let (sender_bal, ops) = fetch_value from_ ([] : operation list) storage in 
        let (recipient_bal, ops) = fetch_value to_ ops storage in 
        // check there is sufficient balance 
        if sender_bal < amt_ then (failwith error_INSUFFICIENT_BALANCE : nat * nat * (operation list)) else
        // give updated balances
        ( abs(sender_bal - amt_), (recipient_bal + amt_), ops) in 
    // update the storage contract 
    let op_update_sender_bal = update_ledger from_ sender_bal storage in 
    let op_update_recipt_bal = update_ledger to_ recipient_bal storage in 
    ( op_update_sender_bal :: (op_update_recipt_bal :: ops_storage_updates) ),
    storage
