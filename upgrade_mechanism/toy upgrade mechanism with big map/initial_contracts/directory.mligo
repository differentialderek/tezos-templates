// the proxy contract for the directory 

// TODO: "pause" on service by governance in the case of a hack | Deactivate
    ...
    | Some old_proxy_addr -> (
        ...
        if p.proxy_name = "storage" then
            ...
            let op_fetch_storage = 
                match (Tezos.get_entrypoint_opt "%fetchInitStorage" new_proxy_addr : unit contract option) with
                | None -> (failwith error_NO_PROXY_FOUND : operation)
...
[@view] let getProxy (proxy_name, storage : string * storage) : address = 
    match Big_map.find_opt proxy_name storage.proxy_addresses with 
    | None -> (failwith error_NO_PROXY_FOUND : address)
    | Some a -> a

[@view] let isProxy (proxy_addr, storage : address * storage) : bool = 
